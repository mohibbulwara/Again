...
import { uploadImage, addDish } from '@/lib/actions';
  ...
  const handleSubmit = (data: ProductFormValues) => {
    ...
    startSubmitTransition(async () => {
      try {
          ...
          const result = await addDish(dishData, user.id);

          if (result.error) {
              throw new Error(result.error);
          }

          toast({ title: 'Dish Added!', description: `${data.name} has been successfully added.` });
          router.push('/dashboard');
      } catch (error: any) {
          console.error("Failed to add dish:", error);
          toast({ title: 'Error', description: error.message || 'Failed to add dish. Please try again.', variant: 'destructive' });
      }
    });
  };
  ...
  return (
    ...
    >
      <Card className="max-w-3xl mx-auto">
        <CardHeader>
          <CardTitle className="font-headline text-2xl">Add a New Dish</CardTitle>
