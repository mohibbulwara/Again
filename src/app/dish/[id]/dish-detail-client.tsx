...
import { ShoppingCart, Loader2, Clock, Mail, Phone, ChevronDown, BadgeAlert, Share2, Eye } from 'lucide-react';
...
export default function DishDetailClient({ dish, seller, relatedDishes }: DishDetailClientProps) {
  const { user, isAuthenticated } = useAuth();
  const { addToCart } = useCart();
  const { t } = useLanguage();
  const { toast } = useToast();
  const [reviews, setReviews] = useState(allReviews.filter(r => r.dishId === dish.id));
  const [isSubmitting, startTransition] = useTransition();
  const [mainImage, setMainImage] = useState(dish.images?.[0] || 'https://placehold.co/600x400.png');

  const averageRating = useMemo(() => {
    if (reviews.length === 0) return dish.rating;
        ...
        >
          ...
          <div className="mt-4 flex items-center gap-4">
             ...
             <div className="flex items-center gap-1.5 text-sm text-muted-foreground">
                ...
                <span>{dish.viewCount || 0} views</span>
             </div>
          </div>
          <p className="mt-6 text-lg text-muted-foreground">{dish.description}</p>
          
          <div className="my-8 flex items-baseline gap-4">
            <div className="text-4xl md:text-5xl font-bold text-primary">৳{dish.price.toFixed(2)}</div>
            {isDiscount && (
              <div className="text-xl md:text-2xl font-medium text-muted-foreground line-through">৳{dish.originalPrice!.toFixed(2)}</div>
            )}
          </div>
          
           <div className="flex items-center gap-2 text-muted-foreground mb-8">
                <Clock className="h-5 w-5"/>
                <span className="font-medium">Estimated Delivery: {dish.deliveryTime}</span>
            </div>
          ...
          <div className="mt-10 flex flex-wrap gap-4">
            <motion.div whileTap={{ scale: 0.95 }}>
                <Button onClick={handleAddToCart} size="lg" className="font-bold text-lg px-8 py-6" disabled={!isAvailable}>
                    {isAvailable ? <>
            ...
            <ShareButton title={dish.name} text={`Check out this delicious ${dish.name} on Chefs' BD!`} />
            {seller && (
              <Collapsible>
